	...
	graphics::Mesh* Generator::flatChunk(int size, int maxVertices) {
		...
		for (int i = 0; i < maxVertices; i++) {
			for (int j = 0; j < maxVertices; j++) {
				...
				uvs.emplace_back(
						(float) j / ((float) maxVertices - 1),
						(float) i / ((float) maxVertices - 1));
		...
		return new graphics::Mesh(vertices, indices, uvs, normals, new graphics::Material(1, vec3(0.05f, 0.05f, 0.05f), vec3(1, 1, 1), vec3(0, 0, 0), 50));
	}
}