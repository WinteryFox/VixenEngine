...
namespace input {
	...
	void Camera::update(graphics::Window *window) {
		...
		horizontal -= sensitivity * window->deltaTime * xOffset;
		vertical -= sensitivity * window->deltaTime * yOffset;
		...
		if (glfwGetKey(window->window, GLFW_KEY_W) == GLFW_PRESS)
			position += direction * window->deltaTime * speed;
		...
		if (glfwGetKey(window->window, GLFW_KEY_SPACE))
			position += vec3(0, 1, 0) * window->deltaTime * speed;
